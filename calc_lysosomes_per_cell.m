function [ distMatrix, lysosome_counts ] = calc_lysosomes_per_cell( lyso_img_data, cell_img_data )
...
redCentroids = reshape([lyso_img_data(:).Centroid], 2, []).';   % Put centroids in format for pdist2
blackCentroids = reshape([cell_img_data(:).Centroid], 2, []).';
...
lysosome_counts = sum(connectedDots,2);

num_cells = length(cell_img_data);
